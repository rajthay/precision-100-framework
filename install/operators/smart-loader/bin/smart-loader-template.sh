CONTAINER=$1
INDEX=$(echo $2 | cut -d ',' -f 1)
FILE_NAME=$(echo $2 | cut -d ',' -f 2)
FILE_TYPE=$(echo $2 | cut -d ',' -f 3)
DATA_FILE_SEPARATOR=$(echo $2 | cut -d ',' -f 4)

LOADER_FILE_NAME=${FILE_NAME}

if test "$SIMULATION_MODE" = "TRUE"; then
   sleep $SIMULATION_SLEEP;
   echo "        END SMART_LOADER ADAPTOR $FILE_NAME"
   exit;
fi

source $PRECISION100_FOLDER/conf/.smart-loader.env.sh

mkdir -p "$SQLLDR_LOG"
mkdir -p "$SQLLDR_BAD"
mkdir -p "$SMART_SQLLDR_CTL_FOLDER";


if [[ -z "$DATA_FILE_SEPARATOR" ]]; then
   DATA_FILE_SEPARATOR=${DEFAULT_DATA_FILE_SEPARATOR:-,}
fi

CONTROL_FILE="$SMART_SQLLDR_CTL_FOLDER/$LOADER_FILE_NAME.ctl"
DATA_FILE="$SQLLDR_INPUT/$LOADER_FILE_NAME.dat"
LOG_FILE="$SQLLDR_LOG/$LOADER_FILE_NAME.log"
BAD_FILE="$SQLLDR_BAD/$LOADER_FILE_NAME.bad"

$PRECISION100_OPERATORS_FOLDER/smart-loader/bin/control-file-generator.sh $LOADER_FILE_NAME $DATA_FILE_SEPARATOR > $CONTROL_FILE 

DIRECT=${DEFAULT_DIRECT:-TRUE}
ERRORS=${DEFAULT_ERRORS:-1000000}
BINDSIZE=${DEFAULT_BINDSIZE:-5048576}
MULTITHREADING=${DEFAULT_MULTITHREADING:-TRUE}
LINES_TO_SKIP=${DEFAULT_LINES_TO_SKIP:-1}

echo "        START SMART_LOADER ADAPTOR $FILE_NAME"


$PRECISION100_FOLDER/bin/audit.sh  $0 "PRE-SMART-LOADER" "$PROJECT_FOLDER / $EXECUTION_NAME / $OPERATION / $SIMULATION_MODE / $CONTAINER / $FILE_NAME" "$PROJECT_FOLDER / $EXECUTION_NAME" "SMART-LOADER" $0 "START"

sqlldr control=$CONTROL_FILE data=$DATA_FILE log=$LOG_FILE bad=$BAD_FILE direct=$DIRECT errors=$ERRORS bindsize=$BINDSIZE multithreading=$MULTITHREADING  skip=${LINES_TO_SKIP} << LOADER 1> >(tee -a $PRECISION100_LOG_FOLDER/loader.log) 2> >(tee -a $PRECISION100_LOG_FOLDER/loader-err.log >&2)
$USERNAME/$PASSWORD@$SID
LOADER

$PRECISION100_FOLDER/bin/audit.sh  $0 "POST-SMART-LOADER" "$PROJECT_FOLDER / $EXECUTION_NAME / $OPERATION / $SIMULATION_MODE / $CONTAINER / $FILE_NAME" "$PROJECT_FOLDER / $EXECUTION_NAME" "SMART-LOADER" $0 "END"

echo "        END SMART_LOADER ADAPTOR $FILE_NAME"
